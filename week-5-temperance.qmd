...
```{r}
plot_predictions(fit_call, type = "prob", by = c("ethnicity", "quality", "city"))
plot_predictions(fit_call, type = "prob", by = c("ethnicity", "quality", "city"), draw = FALSE)

pred_df <- predictions(fit_call, type = "prob", by = c("ethnicity", "quality", "city"), draw = FALSE)
...
# Example assuming your data frame is named `pred_df`
# For demonstration, I'll filter to "yes" group and plot predicted probabilities
pred_df %>%
  filter(group == "yes") %>%
  ggplot(aes(x = quality, y = estimate, fill = ethnicity)) +
  geom_col(position = position_dodge(width = 0.8), color = "black") +
  facet_wrap(~ city) +
  labs(
    title = "Predicted Probabilities by Quality, Ethnicity, and City",
    subtitle = "Model: fit_call — Outcome = 'yes' group",
    x = "Quality Level",
